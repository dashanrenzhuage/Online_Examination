...
public class TestServiceImpl implements TestServiceInterface {
	...
	Result result;*/
	/*@Autowired
	SubCategoryGrade subCategoryGrade;*/
	...
	public void getResult(AnswerSheet answerSheet, Long testId) {
		/*int noOfQuestions = answerSheet.getAnswerList().size();
		int noOfSubcategories = test.getSubCategories().size();
		...
		for (int i = 0; i < noOfQuestions; i++) {
			for (int j = 0; j < 4; j++) {
				if (answerSheet.getAnswerList().get(i).getQuestion().getOpts().get(j).isCorrect() == true) {
					if (answerSheet.getAnswerList().get(i).getAnswer1() == j) {
						totalMarks += 1;
						for (int k = 0; k < 4; i++) {
							if (answerSheet.getAnswerList().get(i).getQuestion().getSubCategory()
		...
		List<SubCategoryGrade> listSubCategoryGrade = new ArrayList<>();
		for (int i = 0; i < noOfSubcategories; i++) {
