...
public class QuestionController {
	 ...
	 * public String generateQuestionsTest(HttpServletRequest
	 * request,@ModelAttribute("newAnswer") AnswerSheet newAnswer, Model model,
	 * HttpSession session) { System.out.println("i m here--------------");
	 * 
	 * Category category = categoryService.getCategoryByID(categoryid); String[]
	 * listOfString = request.getParameterValues("subCategory");
	 * 
	 * List<SubCategory> listOfSubCategory = new ArrayList<>(); for(int i=0;
	 * i<listOfString.length; i++){ SubCategory subCat =
	 * subCatService.getSubCategoryByID(Long.parseLong(listOfString[i]));
	 * listOfSubCategory.add(subCat); } for(SubCategory c: listOfSubCategory){
	 * System.out.println(c.getSubCatName()); }
	 * 
	 * System.out.println("============"); Answer answer= new Answer();
	 * List<Question> questionList=
	 * questionService.getRandomQuestion(listOfSubCategory);
	 * 
	 * List<Answer> answers = new ArrayList<>(); for(int i=0;
	 * i<questionList.size(); i++){ answer.setQuestion(questionList.get(i));
	 * answers.add(answer); } model.addAttribute("questionsList", questionList);
	 * model.addAttribute("answers", answers);
	 * 
	 * questionList.forEach(ql->{ ql.getOpts().forEach(op->{
	 * System.out.println(op.getOptions()); }); });
	 * request.setAttribute("genarateQuestionList", questionList);
	 * session.setAttribute("genarateQuestionList", questionList);
	 * 
	...
	public String addAnswer(@ModelAttribute("newAnswer") AnswerSheet newAnswer, Model model, HttpServletRequest request,
		 ...
		 * request.getAttribute("qenarateQuestionList"); System.out.println(
		 * "*********************************************************************************************"
	...
	public ResponseEntity<List<Question>> generateQuestions(HttpServletRequest request,
		...
		/*
		 * @RequestMapping(value = "/generate", method = RequestMethod.GET)
		 * public void generateQuestions(@RequestParam("category") Long
		 * categoryid, HttpServletRequest request) { System.out.println(
		 * "i m here--------------"); //List<Question> quesList=
		 * questionService.getRandomQuestion(); Category category =
		 * categoryService.getCategoryByID(categoryid); String[] listOfString =
		 * request.getParameterValues("subCategory");
		 * 
		 * List<SubCategory> listOfSubCategory = new ArrayList<>(); for(int i=0;
		 * i<listOfString.length; i++){ SubCategory subCat =
		 * subCatService.getSubCategoryByID(Long.parseLong(listOfString[i]));
